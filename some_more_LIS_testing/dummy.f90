...
!    INTEGER(4), DIMENSION(nmax), INTENT(IN) :: lgs_a_diag_index
!    REAL(8), DIMENSION(nnz), INTENT(IN) :: lgs_a_value
!    REAL(8), DIMENSION(nnz), INTENT(IN) :: lgs_a_valued
!    REAL(8), DIMENSION(nmax), INTENT(IN) :: lgs_b_value
!    REAL(8), DIMENSION(nmax), INTENT(IN) :: lgs_b_valued
!    INTEGER(4), INTENT(OUT) :: ierr
!    REAL(8), DIMENSION(nmax), INTENT(INOUT) :: lgs_x_value
!    REAL(8), DIMENSION(nmax), INTENT(INOUT) :: lgs_x_valued
!    INTEGER(4) :: iter
!    INTEGER(4) :: iter_max
!    INTEGER(4) :: nr, k
!    REAL(8), DIMENSION(nmax) :: lgs_x_value_prev
!    REAL(8) :: temp1, temp2
...
integer(4),                     intent(in) :: nnz, nmax, n_sprs
real(8),                         intent(in) :: omega, eps_sor
integer(4), dimension(nmax+1),  intent(in) :: lgs_a_ptr
integer(4), dimension(nnz),     intent(in) :: lgs_a_index
integer(4), dimension(nmax),    intent(in) :: lgs_a_diag_index
...
real(8),     dimension(nnz),  intent(inout) :: lgs_a_valued
real(8),     dimension(nmax), intent(inout) :: lgs_b_valued
real(8),     dimension(nmax), intent(inout) :: lgs_x_valued
!real(8),     dimension(nmax)                :: incrxd 
integer(4) :: nr, k
real(8), dimension(nmax)                :: lgs_rhs_value
        ...
        !incrxd = 0.
        lgs_x_valued = 0.
        print *, "lgs_b_valued: ", lgs_b_valued
        print *, "rhs: ", lgs_rhs_value
        call SOR_SPRS(lgs_a_value, lgs_a_index, lgs_a_diag_index, &
        &   lgs_a_ptr, lgs_rhs_value, nnz, nmax, n_sprs, omega, eps_sor, &
        &   lgs_x_valued, ierr)
...
&   nrows)
    ...
    REAL(8), DIMENSION(0:nrows), INTENT(IN) :: a0, a2
    REAL(8), DIMENSION(0:nrows), INTENT(IN) :: a0d, a2d
    REAL(8), DIMENSION(0:nrows), INTENT(INOUT) :: a1, b
    REAL(8), DIMENSION(0:nrows), INTENT(INOUT) :: a1d, bd
    REAL(8), DIMENSION(0:nrows), INTENT(OUT) :: x
    REAL(8), DIMENSION(0:nrows), INTENT(OUT) :: xd
    REAL(8), DIMENSION(0:nrows) :: help_x
    REAL(8), DIMENSION(0:nrows) :: help_xd
    INTEGER(4) :: n
    REAL(8) :: temp
    REAL(8) :: temp0
...
&       )
      b(n) = b(n) - temp
    END DO
!         a0(n)  = 0.0 , not needed in the following, therefore
!                           not set
...
!<------------------------------------------------------------------------------
  ...
  real(8), intent(in) :: x1, x2, y1, y2, z11, z12, z21, z22, x, y

  real(8) :: t, u
  real(8) :: bilinint

  real(8), parameter :: I = 1.0
...
!<------------------------------------------------------------------------------
  SUBROUTINE MY_ERFC_STUB_D(x, xd, retval, retvald)
    ...
    END IF
    temp = 1.0/(0.5*z+1.0)
    td = -(temp*0.5*zd/(0.5*z+1.0))
    t = temp
    temp = t*(0.17087277*t-0.82215223) + 1.48851587
    temp0 = t*temp - 1.13520398
    temp1 = t*(t*temp0+0.27886807) - 0.18628806
    temp2 = t*(t*temp1+0.09678418) + 0.37409196
    arg1d = (t*temp2+t*(temp2+t*(t*temp1+t*(temp1+t*(t*temp0+t*(temp0+t*&
&     (temp+t*(0.17087277*t+t*0.17087277-0.82215223)))+&
&     0.27886807))+0.09678418))+1.00002368)*td - 2*z*zd
    arg1 = t*(t*temp2+1.00002368) - z*z - 1.26551223
    temp2 = EXP(arg1)
...
call lis_matrix_create_f(1, lgs_a, ierr)
call lis_vector_create_f(1, lgs_b, ierr)
call lis_vector_create_f(1, lgs_x, ierr)

call lis_matrix_set_size_f(lgs_a, 0, nmax, ierr)
call lis_vector_set_size_f(lgs_b, 0, nmax, ierr)
call lis_vector_set_size_f(lgs_x, 0, nmax, ierr)

do nr=1, nmax

   do nc=lgs_a_ptr(nr), lgs_a_ptr(nr+1)-1
      call lis_matrix_set_value_f(0, nr, lgs_a_index(nc), &
                                               lgs_a_value(nc), lgs_a, ierr)
   end do

   call lis_vector_set_value_f(0, nr, lgs_b_value(nr), lgs_b, ierr)
   call lis_vector_set_value_f(0, nr, lgs_x_value(nr), lgs_x, ierr)

end do

call lis_matrix_set_type_f(lgs_a, 1, ierr)
call lis_matrix_assemble_f(lgs_a, ierr)

!  ------ Solution with Lis

call lis_solver_create_f(solver, ierr)

    ch_solver_set_option = '-i bicgsafe -p jacobi '// &
                           '-maxiter 2000 -tol 1.0e-18 -initx_zeros false'

call lis_solver_set_option_f(trim(ch_solver_set_option), solver, ierr)
call chkerr_f(ierr)

call lis_solve_f(lgs_a, lgs_b, lgs_x, solver, ierr)
call chkerr_f(ierr)

call lis_solver_get_iter_f(solver, iter, ierr)
write(6,'(10x,a,i0)') 'calc_thk_sia_impl: iter = ', iter

lgs_x_value = 0.0
call lis_vector_gather_f(lgs_x, lgs_x_value, ierr)
call lis_matrix_destroy_f(lgs_a, ierr)
call lis_vector_destroy_f(lgs_b, ierr)
call lis_vector_destroy_f(lgs_x, ierr)
call lis_solver_destroy_f(solver, ierr)
...
real(8),     dimension(nmax), intent(inout) :: lgs_b_valued
real(8),     dimension(nmax), intent(inout) :: lgs_x_valued
real(8),     dimension(nmax) :: rhs_value
...
call lis_init_f(ierr)
call lis_matrix_create_f(1, lgs_ad, ierr)
call lis_vector_create_f(1, lgs_bd, ierr)
call lis_vector_create_f(1, lgs_x, ierr)
call lis_vector_create_f(1, rhs, ierr)

call lis_matrix_set_size_f(lgs_ad, 0, nmax, ierr)
call lis_vector_set_size_f(lgs_bd, 0, nmax, ierr)
call lis_vector_set_size_f(lgs_x, 0, nmax, ierr)
call lis_vector_set_size_f(rhs, 0, nmax, ierr)
...
do nr=1, nmax
   ...
   call lis_vector_set_value_f(0, nr, lgs_b_valued(nr), lgs_bd, ierr)

end do

call lis_matrix_set_type_f(lgs_ad, 1, ierr)
call lis_matrix_assemble_f(lgs_ad, ierr)
...
rhs_value = 0.0
call lis_matvec_f(lgs_ad, lgs_x, rhs, ierr) 
call lis_vector_xpay_f(lgs_bd, alpha, rhs, ierr)
call lis_vector_gather_f(rhs, rhs_value, ierr)
...
call lis_matrix_destroy_f(lgs_ad, ierr)
call lis_vector_destroy_f(lgs_bd, ierr)
call lis_vector_destroy_f(lgs_x, ierr)
call lis_vector_destroy_f(rhs, ierr)
call lis_finalize_f(ierr)
